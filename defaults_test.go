...
func TestInit(t *testing.T) {
	...
	if err := SetDefaults(nonPtrVal); err == nil {
	...
	t.Run("DefaultsSetter interface", func(t *testing.T) {
...
func TestCanUpdate(t *testing.T) {
	...
	for input, expect := range pairs {
		output := IsInitialValue(input)
		if output != expect {
			t.Errorf("IsInitialValue(%v) returns %v, expected %v", input, output, expect)
		}
	}
}
